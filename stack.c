...
int sizeof_shm_stack(int size) {
    return (sizeof(STACK) + sizeof(STACK_RECV_BUF) * size);
}
 ...
 */
int init_shm_stack(STACK* p, int stack_size) {
    ...
    p->size = stack_size;
    p->pos  = -1;
    p->items = (STACK_RECV_BUF*) ((char*)p + sizeof(STACK));
    return 0;
}
...
STACK* create_stack(int size) {
    ...
    if ( pstack == NULL ) {
        ...
        char* p = (char*)pstack;
        pstack->items = (STACK_RECV_BUF*) (p + sizeof(STACK));
        pstack->size = size;
        pstack->pos  = -1;
    }
...
int push(STACK* p, STACK_RECV_BUF item) {
    if ( p == NULL ) {
        return -1;
    }
...
int pop(STACK* p, STACK_RECV_BUF* p_item) {
    if ( p == NULL ) {
        return -1;
    }
